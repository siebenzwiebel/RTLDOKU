...
- einer Methode zum Starten des Threads (falls der vorherige Thread fertig ist)
- einer Methode in der entschieden wird, ob eine Lightshow generiert oder regeneriert werden soll
- einer boolean-Variable die angibt, ob der Queue-Thread läuft oder nicht
...
- einem Pointer auf ein Song-Objekt

Das struct kann potentiell in der Zukunft auch benutzt werden, um weitere Parameter für die Generierung einer Lightshow zwischenzuspeichern, falls z.B. die Generierung abhängig von Genres gemacht wird.

Die Lightshow-generating-Queue wurde eingeführt, weil bei jedem Hinzufügen eines Songs ein neuer Thread gestartet wurde, in dem die Lightshow zu den hinzugefügten Songs berechnet wurde. Ohne die Lightshow-generating-Queue hat man bei mehreren, kurz hintereinander hinzugefügten Songs einen Programmabsturz verursacht, da man eine zu große Menge an Threads verursacht hat. Mit der Queue benötigt man nun nur noch einen einzigen, parallel zu allen anderen Threads (GUI etc.), laufenden Thread für die Berechnung mehrerer Lightshows.
